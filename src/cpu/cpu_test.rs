...
mod tests {
    ...
    fn test_flags() {
        let mut c = cpu::Cpu::new();
        c.AF = 0;
        assert!(!c.get_flag(Flag::Z));
        assert!(!c.get_flag(Flag::N));
        assert!(!c.get_flag(Flag::C));
        assert!(!c.get_flag(Flag::H));
    ...
    fn test_push_af() {
        ...
        assert_eq!(t.mem.read_byte(t.cpu.SP), t.cpu.get_f());
    }
